...
STAGE_PROB = {"Descoberta":20,"Proposta":50,"Negociacao":70, WON_STAGE:100, LOST_STAGE:0}
...
def get_pipeline_forecast(period_start=None, period_end=None, owner=None) -> dict:
    ...
    where, params = ["close_date BETWEEN ? AND ?", "stage != ?"], [period_start, period_end, LOST_STAGE]
    ...
    for d in deals:
        ...
        if d["stage"] == WON_STAGE: won += float(d["value"])
...
def get_forecast_by_owner(period_start=None, period_end=None) -> list:
    ...
    with _connect() as c:
        rows = c.execute("""SELECT owner, COUNT(*) AS deal_count, SUM(value) AS pipeline_value,
            ...
            SUM(CASE WHEN stage=? THEN value ELSE 0 END) AS won_value
            FROM deals WHERE close_date BETWEEN ? AND ? AND stage!=?
            GROUP BY owner ORDER BY weighted_forecast DESC""",
            (WON_STAGE, period_start, period_end, LOST_STAGE)).fetchall()
    return [dict(r) for r in rows]

def get_velocity_metrics() -> dict:
    with _connect() as c:
        won = c.execute("SELECT COUNT(*) AS cnt, AVG(value) AS avg_value, SUM(value) AS sum_value FROM deals WHERE stage=?", (WON_STAGE,)).fetchone()
        tc = c.execute("SELECT COUNT(*) AS cnt FROM deals WHERE stage IN (?,?)", (WON_STAGE, LOST_STAGE)).fetchone()
        op = c.execute("SELECT COUNT(*) AS cnt, SUM(value) AS sum_value FROM deals WHERE stage NOT IN (?,?)", (WON_STAGE, LOST_STAGE)).fetchone()
